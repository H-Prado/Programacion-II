...
//                        System.out.println(carSpaces[x][y].toText());
                ...
                for(int x = (int)lowerElectricZone-65; x < carSpaces.length; x++){
                    for(int y = 0; y<carSpaces[0].length; y++) {
                        this.carSpaces[x][y] = new ElectricCarSpace(new Coordinate((char)(x+65),y+1), null);
//                        System.out.println(carSpaces[x][y].toText());
                    }
                }
                inicio = false;
    ...
    public void carEntry (Car car){
        ...
        if(car instanceof CombustionCar) {
            for (int x = 0; x < carSpaces.length; x++) {
                for (int y = 0; y < carSpaces[0].length; y++) {
                    if (carSpaces[x][y].getPlate() == null) {
                        carSpaces[x][y] = new CarSpace(carSpaces[x][y].getCoordinate(), car.getPlate());
                        return;
                    }
                }
            }
            ...
            for (int x = (int)lowerElectricZone-65; x < carSpaces.length ; x++) {
                for (int y = 0; y < carSpaces[0].length; y++) {
                    if (carSpaces[x][y].getPlate() == null){
                        electricCarSpaceAux = new ElectricCarSpace(carSpaces[x][y].getCoordinate(), car.getPlate());
    ...
    public void carDeparture (Car car){
        for(int x = 0; x<carSpaces.length; x++){
            for(int y = 0; y<carSpaces[0].length; y++) {
                if(carSpaces[x][y].getPlate() != null && carSpaces[x][y].getPlate().matches(car.getPlate())) {
                    if(car instanceof CombustionCar) {
                        carSpaces[x][y] = new CarSpace(carSpaces[x][y].getCoordinate(), null);
                        return;
                    }
                    else{
                        carSpaces[x][y] = new ElectricCarSpace(carSpaces[x][y].getCoordinate(), null);
                        return;
                    }
                }
            }
